...
public class PartOneTestCases
{
   ...
   public void testPerimCircle() {
        double d = Util.perimeter(new Circle(new Point(2,0), 4));
        assertEquals(Math.PI*2*4, d, DELTA);
   }

   @Test
   public void testPerimRectangle() {
        double d = Util.perimeter(new Rectangle(new Point(3, 2), new Point(6,0)));
        assertEquals(10, d, DELTA);
   }
   @Test
   public void testWhichIsBigger() {
        List<Point> vertices = Arrays.asList(new Point(0,0), new Point(3, 1), new Point(1,4), new Point(-1,4));
        Circle c = new Circle(new Point(1.0, 1.0), 1.0);
        Rectangle r = new Rectangle(new Point(3,2), new Point(6,0));
